...
export class PositionalProcedure {
  page: Page
  ...
  async procedureCreate(data: ProcedureData) {
    await allure.step('Создать закупку', async () => {
      await this.grid.createNewProcedure()
      await this.container.fillProcedureData(data)
      const procedureCreateData = await this.apiFromUi.procedureData(data)
      data.container.containerUuid = procedureCreateData.containerUuid
      data.procedure.procedureUuid = procedureCreateData.procedureUuid
      data.container.registryNumber = procedureCreateData.registryNumber
    })
  }
}
